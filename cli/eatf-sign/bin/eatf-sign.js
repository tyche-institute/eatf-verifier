...
Usage:
  ...
  eatf-sign --payload <file> --key <pem> --public-key <pem> \\
            ...
            [--pqc-key <seed-pem> --pqc-public-key <spki-pem>] \\
            [--out <file>]
...
Required options:
  ...
  --timestamp <tsr>       RFC 3161 TimeStampResp bytes. The signer does
                          ...
                          the response here.
  --pqc-key <pem>         ML-DSA-65 seed PEM generated by --gen-mldsa.
  --pqc-public-key <pem>  Matching RFC 9881 SubjectPublicKeyInfo PEM.
                          Supplying both creates a hybrid package.
...
Key generation (utility):
  ...
  eatf-sign --gen-mldsa <stem>   Generate <stem>.key (32-byte seed PEM)
                                  and <stem>.pem (RFC 9881 public SPKI).
                                  DEV/test use only.
...
function parseArgs(argv) {
  const args = {
    ...
    genRsa: null, genMldsa: null, printDigest: false,
    pqcKey: null, pqcPublicKey: null,
  };
  for (let i = 2; i < argv.length; i++) {
    ...
    if (a === "--pqc-key") { args.pqcKey = argv[++i]; continue; }
    if (a === "--pqc-public-key") { args.pqcPublicKey = argv[++i]; continue; }
    ...
    if (a === "--gen-mldsa") { args.genMldsa = argv[++i]; continue; }
    if (a.startsWith("--")) {
...
async function main() {
  ...
  if (args.genMldsa) {
    ...
    await writeFile(`${args.genMldsa}.key`, mlDsa65SeedToPem(keypair.seed));
    await writeFile(`${args.genMldsa}.pem`, mlDsa65PublicKeyToPem(keypair.publicKey));
    process.stdout.write(
      `Wrote ${args.genMldsa}.key (ML-DSA-65 seed) and ${args.genMldsa}.pem (RFC 9881 public key)\n`,
    );
    process.stdout.write("NOTE: these are DEV keys. Do NOT use for production attestations.\n");
    return 0;
  }
  ...
  if (Boolean(args.pqcKey) !== Boolean(args.pqcPublicKey)) {
    process.stderr.write(
      "eatf-sign: --pqc-key and --pqc-public-key must be supplied together\n",
    );
    return 2;
  }
  for (const k of ["payload", "key", "publicKey", "metadata", "pqcKey", "pqcPublicKey"]) {
    if (!args[k]) continue;
    if (!existsSync(args[k])) {
      process.stderr.write(`eatf-sign: file not found: ${args[k]}\n`);
      return 2;
  ...
  const signerApi = await loadSigner();
  let pqcSecretKey;
  let pqcPublicKey;
  if (args.pqcKey && args.pqcPublicKey) {
    const seed = signerApi.mlDsa65SeedFromPem(
      await readFile(resolve(args.pqcKey), "utf8"),
    );
    const derived = await signerApi.generateMlDsa65Keypair(seed);
    const suppliedPublic = signerApi.mlDsa65PublicKeyFromPem(
      await readFile(resolve(args.pqcPublicKey), "utf8"),
    );
    if (!Buffer.from(derived.publicKey).equals(Buffer.from(suppliedPublic))) {
      process.stderr.write("eatf-sign: ML-DSA-65 seed and public key do not match\n");
      return 2;
    }
    pqcSecretKey = derived.secretKey;
    pqcPublicKey = derived.publicKey;
  }
  try {
    const result = await signerApi.sign({
