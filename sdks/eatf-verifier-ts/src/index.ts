/**
 * Phase 1 step 1.20: TypeScript offline verifier for EATF .aep
 * evidence packages.
 *
 * v0.1-alpha goal: ship a real verifier that runs in the browser via
 * Web Crypto API and in Node 20+ without any backend round-trip. The
 * pipeline mirrors the Java reference implementation in
 * the AEP profile specification
 * which is the authoritative source of truth per
 * `docs/specs/aep-profile-v1.md` §12.
 *
 * Public API:
 *
 ...
 * symbols but ensures the import path can be statically inlined by a
 * front-end bundler with no Node polyfills.
 */
...
export type VerifyResult = {
  ...
  /**
   * Phase 1.20 v0.2: trust-anchor cross-check result for the embedded
...
export type VerifyOptions = {
  ...
  /**
   * Phase 1.20 v0.2: PEM-encoded root certificates pinned for RFC 3161
